...
function MessageBuilder({ stackData }: any) {
  const [trakkrUrl]: any = useState("https://github.com/skybreakdigital/trakkr-app");
  ...
  const stackTypeMessage = `${Object.keys(stackData.type).length > 1 ? "Split Stack" : "Single"
    } - (${Object.keys(stackData.type)
      .map((key: any) => `${key}: ${stackData.type[key]}`)
      .join(", ")})`;

  const formatMessage = () => {
    const msg = `${discordTag ? discordTag : "{DISCORD_TAG}"} Hey commanders! I have a stack that is completed and ready for sharing. I am available between ${startDate ? startDate : "{START_DATE}"} and ${endDate ? endDate : "{END_DATE}"}.

**Details:**
Stack Size: \`${stackData.size === 20 ? 'Full Stack' : 'Partial Stack'} (${stackData.size} Missions)\`
...
Generated by [Trakkr](<${trakkrUrl}>)
  ...
  return (
    <div className="MessageBuilder flex flex-column gap-3">
      {stackData.size < 20 && (
        <div className="uppercase alert"><i className="fa-solid fa-triangle-exclamation" /> You are sharing only {stackData.size} missions. We suggest sharing a full stack (20), as more Commanders are likely to share with you.</div>
      )}
      <div className="flex gap-3">
        <div className="w-4">
          ...
          <div className="my-3 flex">
            <button className="accent w-full" onClick={onCopyText}><i className="fa-solid fa-copy" /> Copy</button>
          </div>
        </div>
        <div className="w-8">
          <div className="flex flex-column uppercase my-2">
