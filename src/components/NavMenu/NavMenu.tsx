...
function NavMenu() {
  ...
  return (
    <div className="NavMenu flex flex-column justify-content-center align-items-center gap-3">
      <div className="brand">
        <img src={logo} />
      </div>
      {navData.map((data: any, index: number) => (
        <div
          key={index}
          className={`nav-item flex justify-content-center align-items-center ${
            (firstLoadActive && entryHash === data.hash) || isActive(data.url)
              ? "active"
              : ""
